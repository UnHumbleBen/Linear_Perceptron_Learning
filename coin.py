...
def experiment(number_of_coins, number_of_flips):
    v_1 = np.count_nonzero(np.random.randint(2, size=number_of_flips)) / number_of_flips
    random_index = randrange(number_of_coins)
    v_rand = v_1
    v_min = v_1

    for i in range(1, random_index):
        v_new = np.count_nonzero(np.random.randint(2, size=number_of_flips)) / number_of_flips
        if v_new < v_min:
            v_min = v_new

    v_rand = np.count_nonzero(np.random.randint(2, size=number_of_flips)) / number_of_flips
